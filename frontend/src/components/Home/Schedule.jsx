...
const Schedule = ({ darkMode }) => {
  ...
  return (
    <>
      ...
      <Modal
        isOpen={openPayModal}
        toggle={() => setOpenPayModal((prev) => !prev)}
        centered
      >
        <ModalHeader toggle={() => setOpenPayModal((prev) => !prev)}>
          Contact
        </ModalHeader>
        <ModalBody>
          To unlock the contact of the landlord, you must pay Rs 50 upfront.{" "}
          <br /> <strong>This is not refundable amount.</strong>
        </ModalBody>
        <ModalFooter>
